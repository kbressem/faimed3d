# AUTOGENERATED! DO NOT EDIT! File to edit: nbs/02-transforms.ipynb (unless otherwise specified).
...
__all__ = ['Resize3D', 'RandomFlip3D', 'RandomRotate3D', 'RandomRotate3DBy', 'RandomDihedral3D', 'RandomCrop3D',
           ...
           'aug_transforms_3d', 'MaskOneHot', 'SplitVolume']
...
# Cell
@patch
def split_volume(t:(Tensor, TensorDicom3D, TensorMask3D), n_subvolumes=8, cut_along_depth = True):
    """
    separates a 3D tensor into smalle equal sized subvolumes.
    ...
    # calulcate n of subvolumes per dimension
    n = n_subvolumes**(1/3) if cut_along_depth else n_subvolumes**0.5
    n = int(n)

    # check D, H, W and resize if dimensions are uneven
    shape = [s if s % n == 0 else s - s % n for s in t.shape]
    if not cut_along_depth: shape[0]=t.shape[0]
    t =  resize_3d(t, shape)

    # reshape to desired number of subvolumes
    d, h, w = shape

    if cut_along_depth: t = t.reshape(n, int(d/n), n, int(h/n), n, int(w/n))
    else: t = t.reshape(1, d, n, int(h/n), n, int(w/n))

    # flatten the number of subvolumes to one batchdim
    t = t.permute(1, 3, 5, 0, 2, 4).flatten(-3).permute(3, 0, 1, 2)

    return t

@patch
def patch_volume(t:(Tensor, TensorDicom3D, TensorMask3D), n_subvolumes=8, cut_along_depth = True):
    """
    patches a prior split volume back together
    """
    n = n_subvolumes**(1/3) if cut_along_depth else n_subvolumes**0.5
    n = int(n)
    old_shape =[s * n for s in t.shape[1:]]
    if not cut_along_depth: old_shape[0]=t.shape[1]

    t = t.reshape(n,n,n,t.shape[1], t.shape[2], t.shape[3])
    t = t.permute(0,3,1,4,2,5)
    return t.reshape(old_shape)

class SplitVolume(RandTransform):
    split_idx, p = None, 1

    def __init__(self, p=1, n_subvolumes=8, cut_along_depth=True):
        store_attr()
        super().__init__(p=p)
    ...
    def encodes(self, x:(TensorDicom3D, Tensor, TensorMask3D)):
        x = x.split_volume(n_subvolumes=self.n_subvolumes, cut_along_depth=self.cut_along_depth)
        return x if x.device.type == 'cpu' else x.cuda()

    def decodes(self, x:(TensorDicom3D, Tensor, TensorMask3D)):
        x = x.patch_volume(n_subvolumes=self.n_subvolumes, cut_along_depth=self.cut_along_depth)
        return x if x.device.type == 'cpu' else x.cuda()