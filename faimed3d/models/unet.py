...
__all__ = ['ConvTranspose3D', 'UnetBlock3D', 'ResizeToOrig', 'SequentialEx4D', 'AddItems', 'DynamicUnet3D']
...
# Cell
class AddItems(nn.Module):
    "add items in a list"
    def __init__(self):
        super(AddItems, self).__init__()
    def forward(self, inputs:(list, tuple)):
        if isinstance(inputs, tuple): inputs = list(inputs)
        return sum(inputs)
...
class DynamicUnet3D(SequentialEx4D):
    ...
    def __init__(self, encoder, n_out, img_size, n_inp=1, blur=False, blur_final=True, self_attention=False,
        ...
        #concat = Concat(ni*n_inp, ndim = len(img_size)) # replace concat by sum
        add_items = AddItems()
        x = middle_conv(add_items(x))
        layers = [encoder, add_items, middle_conv]
